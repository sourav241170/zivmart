		...
		b = function ( e ) {
			...
			return function ( e, n, i, o ) {
				if ( 0 === e || e ) {
					...
					t.errorHandler(
						new Error( 'Invalid date provided: ' + d )
					);
				}
			};
		};
						...
						if (
							( void 0 !== D.calendarContainer &&
								( ( D.calendarContainer.style.visibility =
									'hidden' ),
								( D.calendarContainer.style.display =
									'block' ) ),
		...
		function T() {
			if ( void 0 !== D.hourElement && void 0 !== D.minuteElement ) {
				var e,
					n,
					t =
						( parseInt( D.hourElement.value.slice( -2 ), 10 ) ||
							0 ) % 24,
					a = ( parseInt( D.minuteElement.value, 10 ) || 0 ) % 60,
					i =
						void 0 !== D.secondElement
							? ( parseInt( D.secondElement.value, 10 ) || 0 ) %
							  60
							: 0;
				void 0 !== D.amPM &&
					( ( e = t ),
		...
		function Z( e ) {
			if (
				! (
					! e ||
					( D.config.minDate &&
						e < D.config.minDate.getFullYear() ) ||
			...
			if ( void 0 !== D.calendarContainer ) {
				...
				if (
					...
					if (
						...
						if (
							( ( D.calendarContainer.style.top = u + 'px' ), h )
						)
							if ( v ) {
								var w = ( function () {
									for (
								...
								d( D.calendarContainer, 'rightMost', ! 1 ),
									...
									( D.calendarContainer.style.left =
										C + 'px' ),
			...
			if ( void 0 !== n ) {
				var t = n,
					...
					i =
						...
						'range' !== D.config.mode;
				if (
					( ( D.selectedDateElem = t ), 'single' === D.config.mode )
				)
					D.selectedDates = [ a ];
		...
		( D.parseDate = b( { config: D.config, l10n: D.l10n } ) ),
			...
			( D.destroy = function () {
				...
				if ( ( ( D._handlers = [] ), D.mobileInput ) )
					...
					D.calendarContainer &&
					D.calendarContainer.parentNode
				)
			...
			( function () {
				( D.element = D.input = g ),
					...
					( function () {
						( D.selectedDates = [] ),
							( D.now =
								D.parseDate( D.config.now ) || new Date() );
						var e =
						...
						if ( D.isMobile )
							return void ( function () {
								...
								( D.mobileInput = s(
									...
									( D.mobileInput.disabled =
										D.input.disabled ),
									( D.mobileInput.required =
										D.input.required ),
