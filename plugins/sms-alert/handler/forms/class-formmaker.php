...
class SAFormMaker extends FormInterface
{
    ...
    public function addSmsalertShortcode($form)
    {
        ...
        $form_enable = smsalert_get_option('formmaker_order_status_' . $form_id, 'smsalert_formmaker_general', 'on');
        $otp_enable  = smsalert_get_option('formmaker_otp_' . $form_id, 'smsalert_formmaker_general', 'on');
        $phone_field = smsalert_get_option('formmaker_sms_phone_' . $form_id, 'smsalert_formmaker_general', '');
    ...
    public function formmakerSubmissionComplete($form )
    {
        ...
        $form_enable      = smsalert_get_option('formmaker_order_status_' . $form_id, 'smsalert_formmaker_general', 'on');
        $phone_field      = smsalert_get_option('formmaker_sms_phone_' . $form_id, 'smsalert_formmaker_general', '');
        $buyer_sms_notify = smsalert_get_option('formmaker_message_' . $form_id, 'smsalert_formmaker_general', 'on');
        $admin_sms_notify = smsalert_get_option('formmaker_admin_notification_' . $form_id, 'smsalert_formmaker_general', 'on');
        if ('on' === $form_enable && 'on' === $buyer_sms_notify) {
            $buyer_sms_content = smsalert_get_option('formmaker_sms_body_'. $form_id, 'smsalert_formmaker_message', '');            
            $datas = $form['fvals'] ;
        ...
        if ('on' === $form_enable && 'on' === $admin_sms_notify ) {
            ...
            if (! empty($admin_phone_number) ) {
                $admin_sms_content = smsalert_get_option('formmaker_admin_sms_body_' . $form_id, 'smsalert_formmaker_message', '');
    ...
    {
        $tabs['formmaker']['nav']  = 'Form Maker';
        $tabs['formmaker']['icon'] = 'dashicons-list-view';

        $tabs['formmaker']['inner_nav']['formmaker_cust']['title']        = 'Customer Notifications';
        $tabs['formmaker']['inner_nav']['formmaker_cust']['tab_section']  = 'formmakertemplates';
        $tabs['formmaker']['inner_nav']['formmaker_cust']['first_active'] = true;
        $tabs['formmaker']['inner_nav']['formmaker_cust']['tabContent']   = array();
        $tabs['formmaker']['inner_nav']['formmaker_cust']['filePath']     = 'views/formmaker_customer_template.php';

        $tabs['formmaker']['inner_nav']['formmaker_admin']['title']       = 'Admin Notifications';
        $tabs['formmaker']['inner_nav']['formmaker_admin']['tab_section'] = 'formmakeradmintemplates';
        $tabs['formmaker']['inner_nav']['formmaker_admin']['tabContent']  = array();
        $tabs['formmaker']['inner_nav']['formmaker_admin']['filePath']    = 'views/formmaker_admin_template.php';

        $tabs['formmaker']['inner_nav']['formmaker_admin']['icon'] = 'dashicons-list-view';
        $tabs['formmaker']['inner_nav']['formmaker_cust']['icon']  = 'dashicons-admin-users';
		$tabs['formmaker']['help_links']                        = array(
    ...
    public static function add_default_settings( $defaults = array() )
    {
        ...
        foreach ( $wpam_statuses as $ks => $vs ) {
            $defaults['smsalert_formmaker_general'][ 'formmaker_admin_notification_' . $ks ] = 'off';
            $defaults['smsalert_formmaker_general'][ 'formmaker_order_status_' . $ks ]       = 'off';
            $defaults['smsalert_formmaker_general'][ 'formmaker_message_' . $ks ]            = 'off';
            $defaults['smsalert_formmaker_message'][ 'formmaker_admin_sms_body_' . $ks ]     = '';
            $defaults['smsalert_formmaker_message'][ 'formmaker_sms_body_' . $ks ]           = '';
            $defaults['smsalert_formmaker_general'][ 'formmaker_sms_phone_' . $ks ]          = '';
            $defaults['smsalert_formmaker_general'][ 'formmaker_sms_otp_' . $ks ]            = '';
            $defaults['smsalert_formmaker_general'][ 'formmaker_otp_' . $ks ]                = '';
            $defaults['smsalert_formmaker_message'][ 'formmaker_otp_sms_' . $ks ]            = '';
        }
        return $defaults;
    }
